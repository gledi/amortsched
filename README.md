...
```
   ...
   app    (use cases + ports, depends only on core)
    ^
    |
 adapters (data-inmemory, data-sqlalchemy, data-mongo, data-tortoise)
    ^
    |
...
| `amortsched-data-tortoise` | `amortsched.data.tortoise` | Tortoise ORM repositories (stub) |
...
└── libs/
    ...
    └── data-tortoise/                       # amortsched.data.tortoise
```
