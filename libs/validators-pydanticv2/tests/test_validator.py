...
from amortsched.validators.vpydanticv2 import PydanticV2Validator
...
def test_register_request_round_trip():
    v = PydanticV2Validator(RegisterRequest)
...
def test_user_response_camel_case():
    v = PydanticV2Validator(UserResponse)
...
def test_create_plan_request_camel_input():
    v = PydanticV2Validator(CreatePlanRequest)
    obj = v.validate(
        {
            "name": "Test",
            "amount": "100000.00",
...
def test_plan_response_camel_output():
    v = PydanticV2Validator(PlanResponse)
    obj = PlanResponse(
        id=_UUID1,
        user_id=_UUID2,
        name="P",
...
def test_validate_raises_validation_error_on_bad_input():
    ...
    assert any("email" in e.get("field", "") for e in exc_info.value.errors)
...
def test_profile_response_camel_output():
    v = PydanticV2Validator(ProfileResponse)
    obj = ProfileResponse(
        id=_UUID1,
        user_id=_UUID2,
        display_name="D",
