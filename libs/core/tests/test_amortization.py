...
from amortsched.core.errors import AmortizationError
from amortsched.core.values import EarlyPaymentFees, InterestRateApplication, PaymentKind
...
def test_rate_change_before_start_date_is_in_effect_for_first_period() -> None:
    ...
    schedule.add_interest_rate_change(datetime.date(2024, 12, 1), 24)

    first_scheduled = _first_scheduled_payment(schedule, start_date=datetime.date(2025, 1, 1))
    days = Decimal("31")
    daily = (Decimal("24") / Decimal("100")) / Decimal("365")
    expected_interest = Decimal("1000") * daily * days
    assert first_scheduled.payment.interest == pytest.approx(expected_interest)
...
def test_invalid_term_raises() -> None:
    with pytest.raises(AmortizationError):
        AmortizationSchedule(amount=1000, term=(-1, 0), interest_rate=10)
